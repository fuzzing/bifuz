...
def parse_logcat(ip, log_filename):
    ...
    if package_name and new_name:
        new_filename = root_path + "/" + activity + "." + new_name
...
def start_intent_fuzzer(ip, log_dir):
    if not os.path.isfile(log_dir + '/all_intent_' + ip + '.sh'):
        print "The intent calls were not generated!"
        return False

    with open(log_dir + '/all_intent_' + ip + '.sh', 'r') as f:
